...
export function ModelsList({ onGoToDB }: ModelsListProps) {
  ...
  const [editing, setEditing] = useState<EditingModel | undefined>(undefined);
  ...
  return (
    <Card className="w-full max-w-3xl shadow-sm">
      <CardHeader>
        ...
        <CardDescription>
          管理模型的多表关系与基本信息;字段与默认值请到「数据库」标签下编辑对应表结构
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {loading && items === null ? (
          ...
          <div className="space-y-3 rounded-lg border border-dashed bg-muted/30 px-4 py-6 text-center">
            ...
            <p className="text-xs text-muted-foreground">
              新建模型前,请先在「数据库」标签中创建表,再回到此处添加表与配置关系
            </p>
        ...
        <p className="text-[11px] text-muted-foreground">
          点击模型即可使用自动生成的界面。点击编辑按钮可修改基本信息、表与关系。
          字段业务类型与默认值请到「数据库」标签下编辑对应表结构。
        </p>
      </CardContent>
