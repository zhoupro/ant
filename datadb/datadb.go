...
// SystemTables are managed by the application itself. Their schema is
// auto-generated by GORM AutoMigrate on first use, so they cannot be edited
// or removed through the generic table API exposed in handlers/tables.go.
// Keep this list in sync with the TableName() declarations in the
// logicmodels, pages, dashboards and logentries packages.
...
// IsSystemTable reports whether the given table name is reserved by the
// application and therefore read-only from the user's perspective.
